...
export const ACHIEVEMENTS = [
  ...
  { id: 'perfect',     icon: '✨', done: (s) => (s.perfectRounds || 0) >= 1 },
  { id: 'hero',        icon: '🦸', done: (s) => s.heroes >= 1 },
  { id: 'hundred',     icon: '💯', done: (s, tot) => tot.attempted >= 100 },
  { id: 'week',        icon: '📅', done: (s) => (s.maxStreak || 0) >= 7 },
  { id: 'gold',        icon: '👑', done: (s, tot, g) => g >= 1 },
...
export function achievementsStatus() {
  ...
  return ACHIEVEMENTS.map((a) => ({ id: a.id, icon: a.icon, title: t.ach[a.id].title, desc: t.ach[a.id].desc, done: !!a.done(s, tot, g) }));
}
